...
const notifications = ref([
  ...
  { id: 3, text: "New Patients in today.", time: "5 mins", read: false },
]);
...
const pageTitle = computed(() => {
  ...
  if (p.includes("/alerts&monitoring")) return "alerts&monitoring";
  if (p.includes("/accesscontrol")) return "accesscontrol";
  if (p.includes("/cctv&surveillance")) return "cctv&surveillance";
  if (p.includes("/patrolmanagement")) return "patrolmanagement";
  if (p.includes("/securitystaff")) return "securitystaff";
  if (p.includes("/securityreports")) return "securityreports";
...
function go(child) {
  navOpen.value = false;
  const to = `/dashboard/security-personnel/${child}`;
  if (route.path === to) return;
  router.push(to).catch(() => {});
}

function isActive(child) {
  return route.path.includes(`/dashboard/security-personnel/${child}`);
}
