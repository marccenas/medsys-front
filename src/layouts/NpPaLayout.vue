  ...
  <div class="ua-shell" :class="{ 'nav-open': navOpen }">
    ...
    <aside class="ua-sidebar">
      <div class="ua-brand">
        ...
        <button
            class="ua-link"
            :class="{ active: isActive('supplies') }"
            @click="go('supplies')"
        >
      ...
      <header class="ua-topbar">
        ...
        <div class="ua-actions">
          ...
          <div class="ua-profile">
            ...
            <div class="ua-user">
              <div class="ua-user-name">Sarah Thompson</div>
              <div class="ua-user-role">Head Nurse / Nurse Unit Manager</div>
            </div>
...
const notifications = ref([
  { id: 1, text: "New shift request submitted.", time: "Just now", read: false },
  { id: 2, text: "Task updated successfully.", time: "10 min ago", read: false },
  { id: 3, text: "Reminder: Review protocols.", time: "1 hr ago", read: true },
]);
...
const pageTitle = computed(() => {
  const p = route.path;
  if (p.includes("/nps-pas")) return "Nurse Practitioners & Physician Assistants";
...
function go(child) {
  navOpen.value = false;
  const to = `/dashboard/np-pa/${child}`;
  if (route.path === to) return;
  router.push(to).catch(() => {});
}

function isActive(child) {
  return route.path.includes(`/dashboard/np-pa/${child}`);
}
