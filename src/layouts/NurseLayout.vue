<template>
  <div class="ua-shell" :class="{ 'nav-open': navOpen }">
    ...
    <aside class="ua-sidebar">
      ...
      <nav class="ua-nav">
        <button class="ua-link" :class="{ active: isActive('dashboard') }" @click="go('dashboard')" type="button">
          <font-awesome-icon icon="gauge" /> <span>Dashboard</span>
        </button>

        <button class="ua-link" :class="{ active: isActive('patients') }" @click="go('patients')" type="button">
          <font-awesome-icon icon="hospital-user" /> <span>Patients</span>
        </button>

        <button class="ua-link" :class="{ active: isActive('tasks') }" @click="go('tasks')" type="button">
          <font-awesome-icon icon="list-check" /> <span>Tasks</span>
        </button>

        <button class="ua-link" :class="{ active: isActive('schedule') }" @click="go('schedule')" type="button">
          <font-awesome-icon icon="calendar-days" /> <span>Schedule</span>
        </button>

        <button class="ua-link" :class="{ active: isActive('learning') }" @click="go('learning')" type="button">
          <font-awesome-icon icon="graduation-cap" /> <span>Learning</span>
        </button>
      </nav>
    </aside>
    ...
    <main class="ua-main">
      <header class="ua-topbar">
        <div class="ua-title">
          ...
          <div class="ua-title-stack">
            <h1>{{ pageTitle }}</h1>
            <div class="ua-subtitle" v-if="subtitle">{{ subtitle }}</div>
          </div>
        </div>
        ...
        <div class="ua-actions">
          ...
          <div class="ua-notif" ref="notifRef" @click.stop>
            ...
            <div class="ua-notif-menu" v-if="notifOpen">
              <div class="ua-notif-head">
                ...
                <button class="ua-notif-action" type="button" @click="markAllRead" :disabled="notifCount === 0">
                  Mark all as read
                </button>
              </div>
          ...
          <div class="ua-profile">
            ...
            <div class="ua-user">
              <div class="ua-user-name">Emily Davis</div>
              <div class="ua-user-role">Registered Nurse</div>
            </div>
      ...
      <section class="ua-content">
        <!-- ✅ ONLY ONE outlet. DO NOT import NurseDashboard here -->
        <router-view />
      </section>
    </main>
  </div>
...
const notifications = ref([
  { id: 1, text: "Vitals update required for Room 102A.", time: "Just now", read: false },
  { id: 2, text: "Medication due in 15 minutes.", time: "10 min ago", read: false },
  { id: 3, text: "New message from Dr. Mitchell.", time: "1 hr ago", read: true },
]);
...
const subtitle = computed(() => {
  const p = route.path;
  if (p.includes("/dashboard")) return "Welcome, Emily Davis";
  return "";
});

function go(child) {
  navOpen.value = false;
  const to = `/dashboard/nurse/${child}`;
  if (route.path === to) return;
  router.push(to).catch(() => {});
}

function isActive(child) {
  return route.path.includes(`/dashboard/nurse/${child}`);
}
...
.ua-notif-title{ font-weight: 900; color: #0f172a; }
.ua-notif-action{
  border: 1px solid rgba(226,232,244,0.95);
  background: #fff;
  border-radius: 10px;
...
.ua-notif-action:disabled{ opacity: .45; cursor: not-allowed; }

.ua-notif-list{ max-height: 300px; overflow: auto; }
.ua-notif-item{
  width: 100%;
  display:flex;
  gap: 10px;
...
.ua-notif-dot.on{ background: #2d76d6; box-shadow: 0 0 0 4px rgba(45,118,214,0.12); }
.ua-notif-msg{ font-weight: 900; color: #0f172a; font-size: 13px; line-height: 1.2; }
.ua-notif-time{ margin-top: 4px; font-size: 11px; color: #64748b; font-weight: 800; }

.ua-notif-empty{ padding: 16px 12px; color: #64748b; font-weight: 900; }
.ua-notif-foot{ padding: 10px 12px; background: rgba(245,248,255,0.65); }
.ua-notif-viewall{
  width: 100%;
  border: 1px solid rgba(226,232,244,0.95);
  background: #fff;
...
@media (max-width: 980px){
  ...
  .ua-shell.nav-open .ua-sidebar{ transform: translateX(0); }
