  ...
  <div class="ua-shell" :class="{ 'nav-open': navOpen }">
    ...
    <aside class="ua-sidebar">
      ...
      <nav class="ua-nav">
        ...
        <button class="ua-link" :class="{ active: isActive('supplies') }" @click="go('supplies')">
...
function go(child) {
  navOpen.value = false;
  const to = `/dashboard/resident/${child}`;
  if (route.path === to) return;
  router.push(to).catch(() => {});
}

function isActive(child) {
  return route.path.includes(`/dashboard/resident/${child}`);
}
